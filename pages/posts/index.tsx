import { TableView, TableViewDataRow } from "@components/data/tableView";
...
const Page: NextPageCustom<PageProps> = (props) =>
{
    const getTableData = (): TableViewDataRow[] => props.postList.map(x =>
    {
        return {
            data: [
                x.isPublished ? 'Published' : 'Not Published',
                x.postName,
                props.projectOptions.find(p => p.projectId == x.projectId)?.projectName || 'Unknown',
                x.postDate.toLocaleString("en-us")
            ],
            event: {
                selector: x.postId,
                onSelectEvent(selector)
                {
                    alert(selector)
                },
            }
    ...
    return (<>
        ...
        <LayoutMain>
            <Typography variant="h4" marginBottom={4}>Posts</Typography>

            <TableView
                headers={["Post Status", "Post Name", "Project", "Post Date"]}
                data={getTableData()}
            />
        </LayoutMain>
    </>)
...
{
    const projectController = new ProjectControllerSS();
    const projectData = await projectController.getOptionItems();

    const postController = new PostControllerSS();
